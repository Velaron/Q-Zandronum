...
DEFINE_ACTION_FUNCTION(AActor, A_MaulerTorpedoWave)
{
	...
	for (int i = 0; i < 80; ++i)
	{
		...
		if ( NETWORK_GetState( ) == NETSTATE_SERVER && torpedo )
			UNLAGGED_SpawnAndUnlagMissile( self, torpedo, false, false );
	}
	self->z = savedz;
}
...
AActor *P_SpawnSubMissile (AActor *source, const PClass *type, AActor *target)
{
	AActor *other = Spawn (type, source->x, source->y, source->z, ALLOW_REPLACE, source->player);
...
DEFINE_ACTION_FUNCTION_PARAMS(AActor, A_FireGrenade)
{
	...
	if (grenadetype != NULL)
	{
		...
		if ( NETWORK_GetState( ) == NETSTATE_SERVER )
		{
			UNLAGGED_SpawnAndUnlagMissile( self, grenade, false, false );
			if ( !NETWORK_ClientsideFunctionsAllowed( self ) )
				SERVERCOMMANDS_MoveThing( grenade, CM_X|CM_Y|CM_VELZ|CM_SETPREV );
		}
	}
}
...
DEFINE_ACTION_FUNCTION(AActor, A_FireSigil3)
{
	...
	for (i = 0; i < 20; ++i)
	{
		...
		if (spot != NULL)
		{
			...
			if ( NETWORK_GetState( ) == NETSTATE_SERVER )
				UNLAGGED_SpawnAndUnlagMissile( self, spot, false, false );
		}
	}
