...
void P_CalcSway (player_t *player, fixed_t angleDelta, fixed_t pitchDelta)
{
	...
	if ( weapon && player->WeaponState & WF_WEAPONBOBBING && !(weapon->WeaponFlags & WIF_DONTBOB) )
	{
		...
		fixed_t SwaySpeed = weapon->SwaySpeed * cl_weaponswayspeed;
		if (SwaySpeed == 0)
		{
			player->swayx = 0;
			player->swayy = 0;
...
void P_MovePlayer_Doom(player_t *player, ticcmd_t *cmd)
{
	...
	{
		if ( zadmflags & ZADF_NEW_FLY_FORMULA )
		{
		...
		{
			if ( ( !P_IsPlayerTotallyFrozen( player ) && !( player->cheats & CF_FROZEN ) ) || player->bSpectating )
			{
